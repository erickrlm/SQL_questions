"""Seed data for the SQL Prep study tool.

All question and dataset definitions live here so database.py
stays focused on schema, CRUD, stats, and sandbox logic.
Each seed function receives get_db() as a dependency to avoid
...
def seed_datasets(get_db):
    ...
    datasets = [
        ...
        {
            "name": "finance",
            "description": "Bank accounts and transactions for window functions, running totals, and date analysis",
            "ddl": """
            ...
            "sample_data": """
                ...
                (1, 'Alice Johnson', 'Checking', 2500.00, '2022-01-15', 'active'),
                (2, 'Alice Johnson', 'Savings', 15000.00, '2022-01-15', 'active'),
                (3, 'Bob Smith', 'Checking', 800.00, '2022-03-20', 'active'),
                (4, 'Bob Smith', 'Credit Card', -450.00, '2022-06-10', 'active'),
                (5, 'Carol Davis', 'Checking', 4200.00, '2021-11-05', 'active'),
                (6, 'Carol Davis', 'Savings', 25000.00, '2021-11-05', 'active'),
                (7, 'Dan Wilson', 'Checking', 150.00, '2023-01-10', 'frozen'),
                (8, 'Eve Brown', 'Checking', 3200.00, '2022-09-01', 'active');
                INSERT INTO transactions VALUES
                (1, 1, 500.00, 'deposit', '2024-01-05', 'Paycheck'),
                (2, 1, -120.50, 'withdrawal', '2024-01-07', 'ATM withdrawal'),
                (3, 1, -45.00, 'withdrawal', '2024-01-08', 'Restaurant'),
                (4, 1, 200.00, 'deposit', '2024-01-15', 'Freelance payment'),
                (5, 1, -350.00, 'withdrawal', '2024-01-20', 'Rent'),
                (6, 2, 1000.00, 'deposit', '2024-01-05', 'Transfer from checking'),
                (7, 3, 300.00, 'deposit', '2024-01-03', 'Paycheck'),
                (8, 3, -80.00, 'withdrawal', '2024-01-04', 'Groceries'),
                (9, 3, -25.00, 'withdrawal', '2024-01-10', 'Coffee shop'),
                (10, 3, -60.00, 'withdrawal', '2024-01-15', 'Gas station'),
                (11, 4, -95.00, 'withdrawal', '2024-01-12', 'Online purchase'),
                (12, 4, 450.00, 'deposit', '2024-01-18', 'Payment'),
                (13, 5, 1200.00, 'deposit', '2024-01-02', 'Paycheck'),
                (14, 5, -200.00, 'withdrawal', '2024-01-06', 'Utilities'),
                (15, 5, -150.00, 'withdrawal', '2024-01-14', 'Insurance'),
                (16, 6, -500.00, 'withdrawal', '2024-01-10', 'Transfer to checking'),
                (17, 1, -95.00, 'withdrawal', '2024-02-01', 'Restaurant'),
                (18, 1, 500.00, 'deposit', '2024-02-05', 'Paycheck'),
                (19, 1, -350.00, 'withdrawal', '2024-02-20', 'Rent'),
                (20, 3, 300.00, 'deposit', '2024-02-03', 'Paycheck'),
                (21, 3, -40.00, 'withdrawal', '2024-02-08', 'Lunch'),
                (22, 5, 1200.00, 'deposit', '2024-02-02', 'Paycheck'),
                (23, 8, 500.00, 'deposit', '2024-01-10', 'Paycheck'),
                (24, 8, -100.00, 'withdrawal', '2024-01-12', 'Phone bill'),
                (25, 8, 250.00, 'deposit', '2024-02-01', 'Side gig');
            """,
        },
    ]
...
def seed_coding_questions(get_db):
    ...
    coding_questions = [
        ...
        # === EASY (4) ===
        {
            ...
            "difficulty": "easy",
            "dataset_reference": "sales",
            "prompt": "Write a query to find all shipped or delivered orders placed in January 2024. Show order_id, customer_id, status, and order_date. Sort by order_date.",
            "correct_answer": "SELECT order_id, customer_id, status, order_date FROM orders WHERE status IN ('shipped', 'delivered') AND order_date BETWEEN '2024-01-01' AND '2024-01-31' ORDER BY order_date",
            "explanation": "Combines an IN filter with a date range. BETWEEN is inclusive on both ends. The ORDER BY ensures a deterministic output. Breaking conditions with AND requires both to be true.",
        },
        {
            "id": "c017",
            "category": "SQL Basics",
            ...
            "difficulty": "easy",
            "dataset_reference": "employees",
            "prompt": "Write a query to list employees sorted by department_id ascending, then by salary descending. Show employee_id, name, department_id, and salary.",
            "correct_answer": "SELECT employee_id, name, department_id, salary FROM employees ORDER BY department_id ASC, salary DESC",
            "explanation": "Multi-column ORDER BY sorts by the first column, then uses the second to break ties. ASC is the default; DESC must be explicit. This is commonly used to see who earns the most within each department.",
        },
        {
            "id": "c018",
            "category": "SQL Basics",
            ...
            "difficulty": "easy",
            "dataset_reference": "university",
            "prompt": "Write a query to count the number of courses offered per department. Show department and course_count. Sort by course_count descending.",
            "correct_answer": "SELECT department, COUNT(*) AS course_count FROM courses GROUP BY department ORDER BY course_count DESC",
            "explanation": "COUNT(*) counts all rows in each group. GROUP BY department creates one row per unique department. ORDER BY course_count DESC shows departments with the most courses first.",
        },
        {
            "id": "c019",
            "category": "SQL Basics",
            ...
            "difficulty": "easy",
            "dataset_reference": "sales",
            "prompt": "Write a query to list all customers and their orders. Include customers with no orders (show NULL for order fields). Show customer_id, name, order_id, and order_date. Sort by customer_id, then order_date.",
            "correct_answer": "SELECT c.customer_id, c.name, o.order_id, o.order_date FROM customers c LEFT JOIN orders o ON c.customer_id = o.customer_id ORDER BY c.customer_id, o.order_date",
            "explanation": "LEFT JOIN keeps every row from the left table (customers). When no matching order exists, order columns become NULL. This is how you list all parents with optional children. INNER JOIN would silently drop customers without orders.",
        },
        # === MEDIUM (16) ===
        {
            "id": "c020",
            ...
            "difficulty": "medium",
            "dataset_reference": "sales",
            "prompt": "Write a query to find the total sales per customer, but only for customers who have spent more than $150 total. Show customer name and total_spent. Sort by total_spent descending.",
            "correct_answer": "SELECT c.name, SUM(o.total) AS total_spent FROM customers c JOIN orders o ON c.customer_id = o.customer_id GROUP BY c.name HAVING SUM(o.total) > 150 ORDER BY total_spent DESC",
            "explanation": "HAVING filters groups after aggregation — a WHERE on SUM(total) would error. The JOIN connects customers to their orders, and GROUP BY collapses orders per customer. Only aggregates can appear in HAVING conditions.",
        },
        {
            "id": "c021",
            "category": "SQL Basics",
            ...
            "difficulty": "medium",
            "dataset_reference": "employees",
            "prompt": "Write a query to classify employees by salary bracket: 'High' if salary >= 90000, 'Medium' if salary >= 60000, 'Low' otherwise. Show employee name, salary, and bracket. Sort by salary descending.",
            "correct_answer": "SELECT name, salary, CASE WHEN salary >= 90000 THEN 'High' WHEN salary >= 60000 THEN 'Medium' ELSE 'Low' END AS bracket FROM employees ORDER BY salary DESC",
            "explanation": "CASE evaluates conditions top-to-bottom; the first match wins. If salary is 95000, it matches the first WHEN and never checks the second. The ELSE catches everything else — without it, unmatched rows get NULL. This pattern avoids nested IFs and is standard across all SQL dialects.",
        },
        {
            "id": "c022",
            "category": "SQL Basics",
            ...
            "difficulty": "medium",
            "dataset_reference": "university",
            "prompt": "Write a query to find pairs of students who share the same major. Show student1, student2, and major. Each pair should appear only once (student1 < student2 alphabetically). Sort by major, then student1.",
            "correct_answer": "SELECT s1.name AS student1, s2.name AS student2, s1.major FROM students s1 JOIN students s2 ON s1.major = s2.major AND s1.name < s2.name ORDER BY s1.major, student1",
            "explanation": "A self-join pairs rows from the same table. s1.name < s2.name ensures each pair appears only once (avoiding A-B and B-A duplicates, and A-A self-pairs). The ON condition matches on major but restricts names so only unique, ordered pairs emerge. Sorting the result makes the output deterministic.",
        },
        {
            "id": "c023",
            "category": "SQL Basics",
            ...
            "difficulty": "medium",
            "dataset_reference": "sales",
            "prompt": "Write a query using UNION to list all unique customer IDs from both: customers who have placed orders, and customers from New York. Show customer_id only. Sort by customer_id.",
            "correct_answer": "SELECT customer_id FROM orders UNION SELECT customer_id FROM customers WHERE city = 'New York' ORDER BY customer_id",
            "explanation": "UNION combines two result sets and removes duplicates. UNION ALL would keep duplicates. Both SELECTs must have the same number of columns. This pattern is useful for merging result sets from different conditions when you need 'A or B' semantics across different tables.",
        },
        {
            "id": "c024",
            "category": "Query Design",
            ...
            "difficulty": "medium",
            "dataset_reference": "sales",
            "prompt": "Write a query using EXISTS to find customers who have at least one cancelled order. Show customer_id and name. Sort by name.",
            "correct_answer": "SELECT customer_id, name FROM customers c WHERE EXISTS (SELECT 1 FROM orders o WHERE o.customer_id = c.customer_id AND o.status = 'cancelled') ORDER BY name",
            "explanation": "EXISTS returns true as soon as the subquery finds a match — it short-circuits for efficiency. The SELECT 1 is conventional (the actual value doesn't matter). This is a correlated subquery: the inner query references c.customer_id from the outer query and is re-evaluated per customer.",
        },
        {
            "id": "c025",
            "category": "Window Functions",
            ...
            "difficulty": "medium",
            "dataset_reference": "employees",
            "prompt": "Write a query to rank employees by salary across the entire company. Show name, salary, and two rankings: one using RANK() and one using DENSE_RANK(). Sort by salary descending.",
            "correct_answer": "SELECT name, salary, RANK() OVER (ORDER BY salary DESC) AS rank_num, DENSE_RANK() OVER (ORDER BY salary DESC) AS dense_rank_num FROM employees ORDER BY salary DESC",
            "explanation": "RANK() skips numbers after ties: 1, 2, 2, 4. DENSE_RANK() doesn't skip: 1, 2, 2, 3. Both give tied rows the same rank. This distinction matters when you need 'top 3 distinct salaries' vs 'top 3 employees.' The entire table is one partition since no PARTITION BY is specified.",
        },
        {
            "id": "c026",
            "category": "Window Functions",
            "topic": "lag-lead",
            "difficulty": "medium",
            "dataset_reference": "finance",
            "prompt": "Write a query to compare each transaction amount with the previous transaction amount for the same account. Show account_id, transaction_date, amount, and previous_amount. Order by account_id, then transaction_date.",
            "correct_answer": "SELECT account_id, transaction_date, amount, LAG(amount) OVER (PARTITION BY account_id ORDER BY transaction_date) AS previous_amount FROM transactions ORDER BY account_id, transaction_date",
            "explanation": "LAG(amount) looks back one row within each account's partition. PARTITION BY account_id resets the window per account. ORDER BY transaction_date ensures the previous row is the chronologically preceding transaction. The first transaction per account has NULL previous_amount since there's nothing before it.",
        },
        {
            "id": "c027",
            "category": "SQL Basics",
            ...
            "difficulty": "medium",
            "dataset_reference": "sales",
            "prompt": "Write a query to show total order amount per customer, split into 'shipped' and 'delivered' columns. Show customer name, shipped_total, and delivered_total. Sort by customer name.",
            "correct_answer": "SELECT c.name, SUM(CASE WHEN o.status = 'shipped' THEN o.total ELSE 0 END) AS shipped_total, SUM(CASE WHEN o.status = 'delivered' THEN o.total ELSE 0 END) AS delivered_total FROM customers c LEFT JOIN orders o ON c.customer_id = o.customer_id GROUP BY c.name ORDER BY c.name",
            "explanation": "This is conditional aggregation using CASE inside SUM. Each SUM only adds the total if the status matches; otherwise it adds 0. LEFT JOIN ensures customers without orders still appear (with 0s). This pattern is the foundation of pivoting data in SQL.",
        },
        {
            "id": "c028",
            "category": "Window Functions",
            ...
            "difficulty": "medium",
            "dataset_reference": "employees",
            "prompt": "Write a query to show each employee's salary as a percentage of the total salary budget. Show name, salary, and salary_pct (rounded to 1 decimal). Sort by salary_pct descending.",
            "correct_answer": "SELECT name, salary, ROUND(salary * 100.0 / SUM(salary) OVER (), 1) AS salary_pct FROM employees ORDER BY salary_pct DESC",
            "explanation": "SUM(salary) OVER () computes the total across all rows — the empty OVER clause means the window is the entire result set. Each row can then reference that total to compute its percentage. A subquery computing the total separately would produce the same result, but the window function is more concise.",
        },
        {
            "id": "c029",
            "category": "Query Design",
            ...
            "difficulty": "medium",
            "dataset_reference": "employees",
            "prompt": "Write a query to find the 3rd highest distinct salary value in the company. Show the salary value.",
            "correct_answer": "SELECT DISTINCT salary FROM employees e1 WHERE 3 = (SELECT COUNT(DISTINCT e2.salary) FROM employees e2 WHERE e2.salary >= e1.salary) ORDER BY salary",
            "explanation": "The correlated subquery counts how many distinct salaries are >= the current row's salary. When that count equals 3, this salary is the 3rd highest. The DISTINCT in the outer query prevents duplicate rows if multiple employees share that salary. Alternative approaches use DENSE_RANK in a CTE or ORDER BY with LIMIT/OFFSET.",
        },
        {
            "id": "c030",
            "category": "SQL Basics",
            ...
            "difficulty": "medium",
            "dataset_reference": "employees",
            "prompt": "Write a query to show each employee's name, salary, and a 'manager_status' column: show 'No Manager' if manager_id is NULL, otherwise show 'Has Manager'. Sort by employee_id.",
            "correct_answer": "SELECT name, salary, CASE WHEN manager_id IS NULL THEN 'No Manager' ELSE 'Has Manager' END AS manager_status FROM employees ORDER BY employee_id",
            "explanation": "CASE WHEN ... IS NULL is the correct way to test for NULL in SQL. = NULL and != NULL don't work — NULL comparisons always yield NULL (unknown), which is not TRUE. COALESCE is for providing defaults for NULL values, not for branching logic. This question tests the fundamental understanding that NULL requires IS NULL / IS NOT NULL operators.",
        },
        {
            "id": "c031",
            "category": "SQL Basics",
            ...
            "difficulty": "medium",
            "dataset_reference": "sales",
            "prompt": "Write a query to show order counts by month for 2024. Show month (as YYYY-MM) and order_count. Sort by month.",
            "correct_answer": "SELECT strftime('%Y-%m', order_date) AS month, COUNT(*) AS order_count FROM orders WHERE order_date >= '2024-01-01' AND order_date < '2025-01-01' GROUP BY month ORDER BY month",
            "explanation": "strftime('%Y-%m', date) extracts year and month for GROUP BY. The WHERE clause restricts to 2024 before grouping. Prefer date range filtering (>= AND <) over wrapping the column in a function in WHERE — it allows index usage. Note the half-open interval: < '2025-01-01' correctly captures all 2024 dates including Dec 31.",
        },
        {
            "id": "c032",
            "category": "SQL Basics",
            ...
            "difficulty": "medium",
            "dataset_reference": "university",
            "prompt": "Write a query to list all enrollments with student name, course name, and grade. Show enrollment_id, student_name, course_name, grade, and semester. Sort by semester, then student_name.",
            "correct_answer": "SELECT e.enrollment_id, s.name AS student_name, c.course_name, e.grade, e.semester FROM enrollments e JOIN students s ON e.student_id = s.student_id JOIN courses c ON e.course_id = c.course_id ORDER BY e.semester, student_name",
            "explanation": "A three-table join chains through foreign keys: enrollments → students (via student_id) and enrollments → courses (via course_id). INNER JOIN is appropriate since every enrollment must have valid student and course references. The order of JOINs doesn't logically matter for INNER JOINs — the planner chooses the execution order.",
        },
        {
            "id": "c033",
            "category": "Query Design",
            ...
            "difficulty": "medium",
            "dataset_reference": "university",
            "prompt": "Write a query to find students who have never taken a course in the Chemistry department. Show student_id and name. Sort by name.",
            "correct_answer": "SELECT student_id, name FROM students s WHERE NOT EXISTS (SELECT 1 FROM enrollments e JOIN courses c ON e.course_id = c.course_id WHERE e.student_id = s.student_id AND c.department = 'Chemistry') ORDER BY name",
            "explanation": "NOT EXISTS with a correlated subquery is the safest way to express exclusion. It correctly handles NULLs — unlike NOT IN, where a single NULL in the subquery result makes the entire condition return no rows. The subquery joins enrollments to courses to filter by department, then checks for any match per student.",
        },
        {
            "id": "c034",
            "category": "Query Design",
            ...
            "difficulty": "medium",
            "dataset_reference": "finance",
            "prompt": "Write a query using a CTE to find accounts where total withdrawals exceed total deposits. Show account_id, customer_name, total_deposits, total_withdrawals, and net. Sort by net (most negative first).",
            "correct_answer": "WITH totals AS (SELECT t.account_id, SUM(CASE WHEN t.transaction_type = 'deposit' THEN t.amount ELSE 0 END) AS total_deposits, SUM(CASE WHEN t.transaction_type = 'withdrawal' THEN ABS(t.amount) ELSE 0 END) AS total_withdrawals FROM transactions t GROUP BY t.account_id) SELECT a.account_id, a.customer_name, totals.total_deposits, totals.total_withdrawals, totals.total_deposits - totals.total_withdrawals AS net FROM totals JOIN accounts a ON totals.account_id = a.account_id WHERE totals.total_withdrawals > totals.total_deposits ORDER BY net",
            "explanation": "The CTE pre-computes deposit and withdrawal totals. The outer query joins to accounts for the customer name. ABS() converts negative withdrawal amounts to positive for display. The WHERE clause filters to accounts spending more than earning. This could be done with a single GROUP BY + HAVING, but the CTE makes the logic clearer and reusable.",
        },
        {
            "id": "c035",
            "category": "Query Design",
            ...
            "difficulty": "medium",
            "dataset_reference": "employees",
            "prompt": "Write a query to find employees who earn more than their direct manager. Show employee name, employee salary, manager name, and manager salary. Sort by salary difference (descending).",
            "correct_answer": "SELECT e.name AS employee, e.salary AS emp_salary, m.name AS manager, m.salary AS mgr_salary FROM employees e JOIN employees m ON e.manager_id = m.employee_id WHERE e.salary > m.salary ORDER BY e.salary - m.salary DESC",
            "explanation": "A self-join with WHERE filters for the 'employee earns more than boss' condition. INNER JOIN is used because we only want employees who have a manager (manager_id IS NOT NULL). The salary difference in ORDER BY shows who out-earns their boss by the widest margin. This pattern is useful for finding anomalies in hierarchical data.",
        },
        # === HARD (10) ===
        {
            "id": "c036",
            ...
            "difficulty": "hard",
            "dataset_reference": "finance",
            "prompt": "Write a query to find consecutive days when each account had at least one transaction. Show account_id, streak_start, streak_end, and days_in_streak for streaks of 3 or more consecutive days. Order by account_id, streak_start. (This is a classic gaps-and-islands problem.)",
            "correct_answer": "WITH daily AS (SELECT DISTINCT account_id, transaction_date FROM transactions), numbered AS (SELECT account_id, transaction_date, julianday(transaction_date) - ROW_NUMBER() OVER (PARTITION BY account_id ORDER BY transaction_date) AS grp FROM daily), streaks AS (SELECT account_id, MIN(transaction_date) AS streak_start, MAX(transaction_date) AS streak_end, COUNT(*) AS days_in_streak FROM numbered GROUP BY account_id, grp) SELECT account_id, streak_start, streak_end, days_in_streak FROM streaks WHERE days_in_streak >= 3 ORDER BY account_id, streak_start",
            "explanation": "The gaps-and-islands technique: number rows per account by date, then subtract the row number from the date. Consecutive dates get the same group value because both the date and row_number increment by 1 each step. Non-consecutive dates create a new group. julianday() converts dates to numeric values for arithmetic. GROUP BY the group identifier finds streaks, and HAVING/WHERE keeps only 3+ day streaks.",
        },
        {
            "id": "c037",
            "category": "Window Functions",
            ...
            "difficulty": "hard",
            "dataset_reference": "finance",
            "prompt": "Write a query to calculate a 3-transaction moving average of deposit amounts per account. Show account_id, transaction_date, amount, and moving_avg (rounded to 2 decimals). Include only deposits. Order by account_id, transaction_date.",
            "correct_answer": "WITH deposits AS (SELECT account_id, transaction_date, amount FROM transactions WHERE transaction_type = 'deposit') SELECT account_id, transaction_date, amount, ROUND(AVG(amount) OVER (PARTITION BY account_id ORDER BY transaction_date ROWS BETWEEN 2 PRECEDING AND CURRENT ROW), 2) AS moving_avg FROM deposits ORDER BY account_id, transaction_date",
            "explanation": "ROWS BETWEEN 2 PRECEDING AND CURRENT ROW defines a sliding window of 3 rows: the current row and the two before it. AVG() over this frame computes the moving average. The first two rows per account average fewer rows (just what's available). PARTITION BY account_id resets the window per account. The CTE first filters to only deposits for cleaner logic.",
        },
        {
            "id": "c038",
            "category": "Query Design",
            ...
            "difficulty": "hard",
            "dataset_reference": "employees",
            "prompt": "Write a recursive CTE to count the total number of direct and indirect reports for each manager. Show manager_id, manager_name, and total_reports. Include only managers who have at least one report. Sort by total_reports descending.",
            "correct_answer": "WITH RECURSIVE hierarchy AS (SELECT employee_id, manager_id FROM employees WHERE manager_id IS NOT NULL UNION ALL SELECT e.employee_id, e.manager_id FROM employees e JOIN hierarchy h ON e.manager_id = h.employee_id) SELECT m.employee_id AS manager_id, m.name AS manager_name, COUNT(*) AS total_reports FROM hierarchy h JOIN employees m ON h.manager_id = m.employee_id GROUP BY m.employee_id, m.name ORDER BY total_reports DESC",
            "explanation": "The recursive CTE builds all manager-employee relationships at every depth. The anchor finds all employees with a manager. The recursive step follows the chain upward: for each employee found, find their manager. The final aggregation counts all records per manager. This counts indirect reports (depth > 1) as well as direct ones. The CTE traverses up the hierarchy (employee → their manager), so grouping by manager_id sums all descendants.",
        },
        {
            "id": "c039",
            "category": "Query Design",
            ...
            "difficulty": "hard",
            "dataset_reference": "sales",
            "prompt": "Write a query to pivot order totals by status for each customer. Show customer name and four columns: shipped, delivered, processing, and cancelled (each showing the sum of order totals for that status). Sort by customer name.",
            "correct_answer": "SELECT c.name, SUM(CASE WHEN o.status = 'shipped' THEN o.total ELSE 0 END) AS shipped, SUM(CASE WHEN o.status = 'delivered' THEN o.total ELSE 0 END) AS delivered, SUM(CASE WHEN o.status = 'processing' THEN o.total ELSE 0 END) AS processing, SUM(CASE WHEN o.status = 'cancelled' THEN o.total ELSE 0 END) AS cancelled FROM customers c LEFT JOIN orders o ON c.customer_id = o.customer_id GROUP BY c.name ORDER BY c.name",
            "explanation": "This is a manual pivot using conditional aggregation. Each column uses SUM(CASE WHEN status = X THEN total ELSE 0 END) to aggregate only matching rows. LEFT JOIN ensures customers with no orders still appear (with 0s). This technique works in all SQL databases — unlike PIVOT syntax which is vendor-specific (SQL Server, Snowflake).",
        },
        {
            "id": "c040",
            "category": "Window Functions",
            ...
            "difficulty": "hard",
            "dataset_reference": "employees",
            "prompt": "Write a query to find the top 3 salary ranks per department, including ties. Use DENSE_RANK so that if multiple employees share rank 3, all are included. Show department name, employee name, salary, and rank. Sort by department name, then rank.",
            "correct_answer": "WITH ranked AS (SELECT d.department_name, e.name, e.salary, DENSE_RANK() OVER (PARTITION BY e.department_id ORDER BY e.salary DESC) AS rnk FROM employees e JOIN departments d ON e.department_id = d.department_id) SELECT department_name, name, salary, rnk FROM ranked WHERE rnk <= 3 ORDER BY department_name, rnk",
            "explanation": "DENSE_RANK ensures consecutive ranking without gaps. If three employees tie for rank 1, the next distinct salary gets rank 2. The WHERE rnk <= 3 filter includes all ties at rank 3. Compare with ROW_NUMBER (never ties) and RANK (ties but skips numbers). The CTE separates ranking from filtering — essential since window functions can't go in WHERE directly.",
        },
        {
            "id": "c041",
            "category": "Query Design",
            ...
            "difficulty": "hard",
            "dataset_reference": "university",
            "prompt": "Write a query to find students whose average grade (on a 4.0 scale: A=4.0, A-=3.7, B+=3.3, B=3.0, B-=2.7, C+=2.3) is higher than the overall average GPA. Show student name, their GPA (rounded to 2 decimals), and the overall average GPA. Sort by GPA descending.",
            "correct_answer": "WITH grade_points AS (SELECT e.student_id, s.name, CASE e.grade WHEN 'A' THEN 4.0 WHEN 'A-' THEN 3.7 WHEN 'B+' THEN 3.3 WHEN 'B' THEN 3.0 WHEN 'B-' THEN 2.7 WHEN 'C+' THEN 2.3 ELSE NULL END AS points FROM enrollments e JOIN students s ON e.student_id = s.student_id), student_gpas AS (SELECT student_id, name, ROUND(AVG(points), 2) AS gpa FROM grade_points GROUP BY student_id, name) SELECT name, gpa, (SELECT ROUND(AVG(gpa), 2) FROM student_gpas) AS overall_avg_gpa FROM student_gpas WHERE gpa > (SELECT AVG(gpa) FROM student_gpas) ORDER BY gpa DESC",
            "explanation": "This uses multiple CTEs and scalar subqueries. The first CTE converts letter grades to numeric points with CASE. The second averages per student. The outer query computes the overall average via a scalar subquery and compares each student's GPA. This is a classic 'above average' problem combining CASE, CTEs, and subqueries. The double reference to the same CTE shows why CTEs beat subqueries here.",
        },
        {
            "id": "c042",
            "category": "Window Functions",
            ...
            "difficulty": "hard",
            "dataset_reference": "employees",
            "prompt": "Write a query to divide employees into 4 salary quartiles (1 = highest paid group, 4 = lowest paid group). Show name, salary, and quartile. Sort by quartile, then salary descending.",
            "correct_answer": "SELECT name, salary, NTILE(4) OVER (ORDER BY salary DESC) AS quartile FROM employees ORDER BY quartile, salary DESC",
            "explanation": "NTILE(N) distributes rows as evenly as possible into N buckets based on the ORDER BY. With 12 employees and 4 buckets, each gets exactly 3 rows. If the row count isn't divisible by N, earlier buckets get the extra rows. NTILE is useful for percentile analysis, A/B testing cohorts, and performance tier classification.",
        },
        {
            "id": "c043",
            "category": "Window Functions",
            ...
            "difficulty": "hard",
            "dataset_reference": "finance",
            "prompt": "Write a query to compute the running balance for each account after each transaction. Show account_id, transaction_date, description, amount, and running_balance. Order by account_id, transaction_date.",
            "correct_answer": "SELECT account_id, transaction_date, description, amount, SUM(amount) OVER (PARTITION BY account_id ORDER BY transaction_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_balance FROM transactions ORDER BY account_id, transaction_date",
            "explanation": "The running balance is a cumulative sum: SUM(amount) with a frame from the partition start to the current row. Withdrawals are negative, deposits positive, so the running sum naturally reflects the account balance after each transaction. PARTITION BY account_id restarts the running sum per account. This pattern is essential for account statements, inventory tracking, and any cumulative time-series analysis.",
        },
        {
            "id": "c044",
            "category": "Query Design",
            ...
            "difficulty": "hard",
            "dataset_reference": "employees",
            "prompt": "Write a query to compute the median salary per department. Show department_name and median_salary (rounded to 2 decimals). Sort by department_name. (Hint: for an even number of values, average the two middle values.)",
            "correct_answer": "WITH ranked AS (SELECT d.department_name, e.salary, ROW_NUMBER() OVER (PARTITION BY d.department_name ORDER BY e.salary) AS rn, COUNT(*) OVER (PARTITION BY d.department_name) AS cnt FROM employees e JOIN departments d ON e.department_id = d.department_id) SELECT department_name, ROUND(AVG(salary), 2) AS median_salary FROM ranked WHERE rn IN ((cnt + 1) / 2, (cnt + 2) / 2) GROUP BY department_name ORDER BY department_name",
            "explanation": "The median is found by: (1) numbering rows per department sorted by salary, (2) computing the count per department, (3) selecting the middle row(s). For odd counts, (cnt+1)/2 and (cnt+2)/2 produce the same integer — picking the middle row. For even counts, they produce two different integers — averaging the two middle values. ROUND(AVG()) handles both cases cleanly. Integer division in SQLite means (cnt+1)/2 correctly rounds down.",
        },
        {
            "id": "c045",
            "category": "Query Design",
            ...
            "difficulty": "hard",
            "dataset_reference": "finance",
            "prompt": "Write a query using multiple CTEs to find accounts whose average daily balance in January 2024 was below $500. Show account_id, customer_name, and avg_daily_balance (rounded to 2 decimals). Sort by avg_daily_balance ascending.",
            "correct_answer": "WITH dates AS (SELECT DISTINCT transaction_date FROM transactions WHERE transaction_date BETWEEN '2024-01-01' AND '2024-01-31'), grid AS (SELECT a.account_id, a.customer_name, d.transaction_date FROM accounts a CROSS JOIN dates d), daily_balance AS (SELECT g.account_id, g.customer_name, g.transaction_date, COALESCE(SUM(t.amount) OVER (PARTITION BY g.account_id ORDER BY g.transaction_date ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW), 0) AS balance FROM grid g LEFT JOIN transactions t ON g.account_id = t.account_id AND g.transaction_date = t.transaction_date) SELECT account_id, customer_name, ROUND(AVG(balance), 2) AS avg_daily_balance FROM daily_balance GROUP BY account_id, customer_name HAVING AVG(balance) < 500 ORDER BY avg_daily_balance",
            "explanation": "This multi-CTE pipeline: (1) extracts all dates in January, (2) cross-joins with accounts for a complete date-per-account grid, (3) computes daily running balances by left-joining actual transactions against the grid — days with no transaction get balance 0 via COALESCE, (4) averages the daily balances and filters below $500. The date-spine + cross join pattern is essential for financial reporting where missing days must still be represented.",
        },
    ]
